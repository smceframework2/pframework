...
class QueueListen
{
    ...
    */
	...
	public function start()
	{
		...
		while(true)
		{
			...
			$status=self::getStatus(self::queKey);
			$statusInt=substr(status,0,1);
			...
			{
				$sleepInt=(int)substr(status,2,strlen(status));
    ...
    */
	...
	private function transactions(time)
	{
		...
		$arr=QueueAdapter::get(self::queKey);
    ...
    */
	public function status()
	{
		...
		if statusInt==1
		{
			...
			$onPointTime=(int)self::getOnPoint(self::queKey);
