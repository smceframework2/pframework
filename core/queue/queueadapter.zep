...
class QueueAdapter
{
	...
	public static function set(queKey,key,$class,duration,time)
	{
		...
		if self::adapterName=="Memcache"
		{
			...
			if !empty(arr) 
        	{
        		...
        		$newArr[key]=["class":$class,"time":time,"duration":duration];
    ...
    */
	...
	{	
		...
		$arr=unserialize(self::adapter->get(self::prefix.queKey));
